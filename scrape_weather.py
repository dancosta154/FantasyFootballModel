...
def scrape_weather():
    ...
    for file in file_list:
        ...
        df2["Temp"] = df2["Forecast"].map(
            lambda x: re.search(r"(\d+)", x)[0] if x != "DOME" else 0
        )
        df2["Weather_Desc"] = df2["Forecast"].map(
            lambda x: re.search(r"\s(.+)", x)[0].strip() if x != "DOME" else 0
        )
        df2["Wind"] = np.where(df2["Forecast"] == "DOME", 0, df2["Wind"])
        df2["Wind_Speed_MPH"] = np.where(
            df2["Forecast"] == "DOME", 0, df2["Wind_Speed_MPH"]
        )
        df2["Wind_Direction"] = np.where(
            df2["Forecast"] == "DOME", 0, df2["Wind_Direction"]
        )
        dfs.append(df2)
